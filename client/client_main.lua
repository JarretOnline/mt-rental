...
CreateThread(function()
    for k, v in pairs(Config.RentalSpots) do
        ...
        exports[Config.QBTargetName]:AddBoxZone(v.SpotName, v.PedCoords, 4, 4, {
            name=v.SpotName,
...
RegisterNetEvent('mt-rental:client:RentVehicleInput', function(data)
    if IsRenting == false then
        ...
        exports['okokNotify']:Alert("RENTAL", "VEHICLE OUT!", 4000, 'error')
    end
end)
    ...
    QBCore.Functions.TriggerCallback("mt-rental:server:VerifyMoney", function(hasMoney)
        if (hasMoney) then
            if IsRenting == false then
                ...
                if IsRenting == true then
                    ...
                    if IsRenting == true then
                            TriggerServerEvent('qb-phone:server:sendNewMail', {
                              ...
                              exports['okokNotify']:Alert("RENTAL", "EXPIRES: 01 MIN", 2000, 'warning')
                              Citizen.Wait(60000)
                              exports['okokNotify']:Alert("RENTAL", "EXPIRES: 10 SEC", 2000, 'warning')
                              Citizen.Wait(10000)
                ...
                exports['okokNotify']:Alert("RENTAL", "LOOK FOR VEHICLE!", 6000, 'success')
            elseif IsRenting == true then
                exports['okokNotify']:Alert("RENTAL", "VEHICLE OUT!", 2000, 'error')
            end
        else
            exports['okokNotify']:Alert("RENTAL", "NO MONEYS!", 2000, 'error')
            return
        end
    end, data.RentPrice)
end)
...
RegisterNetEvent('mt-rental:client:CancelRenting', function(data)
    ...
    if IsRenting == true then
        ...
        exports['okokNotify']:Alert("RENTAL", "NO RENTAL!", 2000, 'error')
    end
end)
